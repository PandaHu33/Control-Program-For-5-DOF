...
////////////////////////////////////////////
    ...
    while (ros::ok)
    {
        ...
        ROS_INFO("Step111111111111111111111111111111111111");
        ...
        ROS_INFO("Step2222222222222222222222222222222222222");
        //关节4、5指令限位
        if(control_type=="PID")
        {   
            if(q(0,0) < -PI/2){
        ...
        //发送控制指令
        ROS_INFO("Step3333333333333333333333333333333333333");
        can_msgs::Frame frames;
        //若q经过初始化全为零还未读到真实的帧，则不发送控制指令
        if(q(0,0)==0.00001 && q(1,0)==0.00001 && q(2,0)==0.00001){
        }else{
        ...
        ROS_INFO("Step44444444444444444444444444444444444");
...
void MainpulatorCallback(const can_msgs::Frame& receive_message)
{
    ROS_INFO("Mainpulatorcallback Executed!!!!");
    if(control_type=="PID")
    {
...
void QGC_Callback(const sensor_msgs::JointState & msg){
    ...
    ROS_INFO("QGC Response Executed!!!!!!!");
}
