...
void InverseRotationCallback(const sensor_msgs::JointState &msg);
...
int main(int argc, char *argv[])
{
    ...
    ros::NodeHandle R_nh;
    ros::Subscriber R_nh_sub = R_nh.subscribe("/fusion/pose_fused", 100, InverseRotationCallback);
    ...
    while (ros::ok)
    {
        ...
        vector<double> actual_angles(4, 0);
        //转化为角度
        for (int i = 0; i < 3; i++)
        {
            actual_angles[i] = q(i,0) * 180 / 3.1415926535;
        }
        actual_angles[3] = joint4_actual_angle * 180 / 3.1415926535;
        //正运动学求解得转换矩阵
        vector<double> endPose;
        PoseMatrix basis_to_end_transmat = KinematicsTarget.ForwardKinematics(actual_angles, endPose);
        PoseMatrix basis_to_camera_transmat = basis_to_end_transmat * TransMat(0,-40,-40) * RotMat(0,0,1.57);
        //basis_to_end_transmat.Display();
        //RotMat(0,0,1.57).Display();
        //basis_to_camera_transmat.Display();
        for(int i=0; i<4; i++){
            for(int j=0; j<4; j++){
                joint_state.effort[i*4+j] = basis_to_camera_transmat.elements[i][j];
            }
        }
        ...
        if (control_type == "PID")
        {
            // 一关节限位，考虑仿ROV平台（原先为-pi/7 ~ 9pi/12 ....?）
            if (expect_q(0, 0) < -PI *2.0/3.0 )
            {
                expect_q(0, 0) = -PI *2.0/ 3.0;
            }
            else if (expect_q(0, 0) > 3.0 * PI / 4.0)
            {
                expect_q(0, 0) = 3.0 * PI / 4.0;
            }
            if (joint4angle < -6.2)
            {
                joint4angle = -6.2;
...
void InverseRotationCallback(const sensor_msgs::JointState &msg){
    R_B02B = RotMat(vector<double>{msg.velocity[0], msg.velocity[1], msg.velocity[2]});
    // double element[4][4] = {{msg.velocity[0], msg.velocity[1], msg.velocity[2],0},
    //                     {msg.velocity[3], msg.velocity[4], msg.velocity[5],0},
    //                     {msg.velocity[6], msg.velocity[7], msg.velocity[8],0},
    //                     {0,0,0,1}};
    // R_B02B = PoseMatrix(element);
...
void decide_and_plan()
{
    ...
    // 抓小车的预备轨迹
    static jointspace_seq_traj traj_from_kaiji_to_extended({joint_command({0, 1}, {1.57, 3.0/4.0*PI}),
                                                            joint_command(2, 1.0/4.0*PI)},
                                                           0.05);
    ...
    // 放置小车的轨迹
    static jointspace_seq_traj traj_from_extended_to_place({joint_command(1, 1.0/6.0*PI),
                                                            joint_command(2, -1.0/6.0*PI),
                                                            joint_command(1, 1.0/4.0*PI),
                                                            joint_command(2, -1.0/4.0*PI)},
                                                           0.05);
    ...
    else if (RE_status == 4)
    {
        ...
        //     joint_command(4, 0.6)
        // },0.05);
        jointspace_seq_traj traj_extended_ultimate({joint_command(0, 1.0/2.0*PI),
                                                    joint_command(1, 3.0/4.0*PI),
                                                    joint_command(2, 1.0/4.0*PI)},
                                                   0.05);
        if (RE_status_changed)
        {
    ...
    else if (RE_status == 5)
    {
        ...
        jointspace_seq_traj traj_place_ultimate({joint_command(1, 1.0/4.0*PI),
                                                 joint_command(2, -1.0/4.0*PI)},
                                                0.05);
        ...
        ROS_INFO("has_complete_place extended: %d", has_complete_place);
        ...
        //从抓小车状态开始放置
        if (Status4_lock == 1 || (abs(q(0, 0) + 1.0/2.0*PI) < 0.05 && abs(q(1, 0) - 1.0/4.0*PI) < 0.05 && abs(q(2, 0) + 1.0/4.0*PI) < 0.08 && Status4_lock == 0))
        {
            ROS_INFO("traj from extended to place");
            Status4_lock = 1;
            has_complete_place = traj_from_extended_to_place.run();
            ROS_INFO("has_complete_place: %d", has_complete_place);
    ...
    else if (RE_status == 6)
    {
        ...
        vector<double> q_actual = {q(0,0)/PI*180,q(1,0)/PI*180,q(2,0)/PI*180,joint4_actual_angle/PI*180};
        static PoseMatrix Tfk;
        if (RE_status_changed)
        {
            ROS_INFO("status==changed ");
            Status4_lock = 0;
            Tfk = robot.ForwardKinematics(q_actual, endPose);
        }

        vector<double> endPose;
        PoseMatrix Tfk_swayed = inv(R_B02B) * Tfk;
        vector<double> endPositionDesired = {Tfk_swayed.elements[0][3], Tfk_swayed.elements[1][3], Tfk_swayed.elements[2][3]};
        vector<double> expected_angles(4);
        ROS_INFO("IK solver input: %lf, %lf, %lf", endPositionDesired[0], endPositionDesired[1], endPositionDesired[2]);
        robot.InverseKinematics3DofPosition(expected_angles, endPositionDesired, q_actual, 0.0);
        // qd(0,0) = expected_angles[0]/180.0*PI;
        // qd(1,0) = expected_angles[1]/180.0*PI;
        // qd(2,0) = expected_angles[2]/180.0*PI;
        expect_q(0,0) = expected_angles[0]/180.0*PI;
        expect_q(1,0) = expected_angles[1]/180.0*PI;
        expect_q(2,0) = expected_angles[2]/180.0*PI;
        // joint4angle = q(3,0);
        joint4angle = expected_angles[3]/180.0*PI;
        ROS_INFO("IK solution: %lf, %lf, %lf, %lf", expect_q(0,0), expect_q(1,0), expect_q(2,0), joint4angle);
    ...
    else if (RE_status == 8)
    {
        ...
        //更改期望角度为接收到的角度
        expect_q(0, 0) = visual_received_angle[0];
        expect_q(1, 0) = visual_received_angle[1];
        expect_q(2, 0) = visual_received_angle[2];
        joint4angle = visual_received_angle[3];
