...
void callback(const sensor_msgs::JointState& joint_state)
{
    ...
    for(int i=0;i<3;i++)
    {
        ...
        jointActualAngles[i]  = joint_state.position[i+1]/3.14*180;
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber mpul_sub_ = mainpulator.subscribe("Angle_Pub_To_All_Nodes",100,callback);
	...
	while (ros::ok)
	{
		...
		i_count++;
		msgMotorTemplate motorData = msgMotorTemplate(i_count, 0, jointActualAngles, 0, {0, 0, 0, 0, 0}, "Motor data for test!");
		//display(motorData);
		//send(connectfd, "1111",4,0);
		 if(!send(connectfd, (stdMsgMotor(motorData).char_msg), 256, 0)){
		// if(!send(connectfd, "1111", 4, 0)){
			cout<<"Send Error!"<<endl;
		 }

		//size_recv = recv(connectfd, buf, sizeof(buf), MSG_DONTWAIT);
		size_recv = recv(connectfd, buf, sizeof(buf), 0);
		if (size_recv <= 0)
		{
			continue;
		}
		else
		{
			...
			{
				ROS_INFO("RECV LEN = %d", size_recv);
				for (int i = 0; i < 256; i++)
				{
					//msg_char[i] = char();
					msg_char[i] = buf[i];
					//msg_char[i] = buf[255-i];
					//cout<<int(msg_char[i])<<" ";
				}
				cout<<endl;
