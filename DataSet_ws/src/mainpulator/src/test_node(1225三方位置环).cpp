    ...
    while (ros::ok)
    {
        ...
        */
        ...
        if(KB_D>0.01)
        {
           ...
           //tol5 = 30;
            tol5 = 45;
            if(joint5_actual_angle>6.2)  //6 //测量抓手距离5cmm时对应的角度值
            {
                tol5 = 0;
...
void ImitationCallback(const sensor_msgs::JointState::ConstPtr& msg)
{
    ...
    {
        ROS_WARN("Received imitation trajectory message with incorrect dimensions. Position array should have 4 elements (q0,q1,q2,gripper_flag).");
        return;
    }
    ...
    // 【新增】从 position[3] 获取抓手信号并更新全局变量 KB_D
    KB_D = msg->position[3];
    ...
    // 取消下面这行代码的注释以进行调试
    // ROS_INFO("Received new trajectory: q=[%f,%f,%f], gripper=%f", expect_q(0,0), expect_q(1,0), expect_q(2,0), KB_D);
}
