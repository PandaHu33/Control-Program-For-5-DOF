# 显控界面按钮说明

### 硬件操作流程

##### 1.机械臂电源拨到右边上电

![31259623543b830b3ad0c663202dd5e](https://gitee.com/iiimp1/images/raw/master/imgs/31259623543b830b3ad0c663202dd5e.jpg)

##### 2.灵巧手电池连接上电

![69d10d890d364f8cb9e5fb2f33a7125](https://gitee.com/iiimp1/images/raw/master/imgs/69d10d890d364f8cb9e5fb2f33a7125.jpg)

##### 3.点击手套按钮，绿灯快闪说明已经连接

![手套(1)](https://gitee.com/iiimp1/images/raw/master/imgs/%E6%89%8B%E5%A5%97(1).jpg)

##### 4.打开“主端程序beta for 5-dof”文件夹，点击程序 一键启动显控启动程序

![image-20260502112623840](https://gitee.com/iiimp1/images/raw/master/imgs/image-20260502112623840.png)

##### 5.选择允许使用相机![image-20260502112656691](https://gitee.com/iiimp1/images/raw/master/imgs/image-20260502112656691.png)

此时会弹出另外3个窗口，保持在后端即可

##### 6. 成功启动会显示如下界面：

![image-20260502112900532](https://gitee.com/iiimp1/images/raw/master/imgs/image-20260502112900532.png)

### 双摄像头模式

- 默认使用“显控显示”：浏览器上下显示两路摄像头，不启动 RTSP/HLS 推流；首次使用需要允许浏览器访问摄像头。
- 在“双摄像头”卡片中选择“显控显示”或“推流”，点击“保存为下次启动模式”，关闭并重新运行一键启动后生效。
- “推流”模式会将两路画面上下拼接，并在网页中同步监看。RTSP 地址为 `rtsp://192.168.3.10:8554/usb_camera`，本机 HLS 地址为 `http://localhost:8081/usb_camera/index.m3u8`。
- 若只显示一路，请检查对应状态提示、USB 连接和浏览器摄像头权限；另一路不会因此被关闭。

##### 默认为键盘控制，点击手柄可切换为手柄控制

##### 使用手柄控制前，确保下方文字显示xbox 360 Controller(说明手柄已经连接) 否则请勿切换手柄控制！！！

![image-20260502133821123](https://gitee.com/iiimp1/images/raw/master/imgs/image-20260502133821123.png)

##### 若显示已达到工作空间外边界，目标被限制，需要往回运动![image-20260502133101782](https://gitee.com/iiimp1/images/raw/master/imgs/image-20260502133101782.png)

##### 若显示已达到内肘/自碰安全边界，目标被限制，则需要往外运动

![image-20260502133553504](https://gitee.com/iiimp1/images/raw/master/imgs/image-20260502133553504.png)

具体控制方式如下：

- `位置控制`：按末端 XYZ 位置目标控制。
- `关节控制`：按三个关节目标控制。
- `W/A/S/D`：键盘模式下控制前后左右。
- `Q/E`：键盘模式下控制上下。
- `左摇杆`：手柄模式下控制前后左右。
- `L/L2`：手柄模式下控制上下，`L` 为下降，`L2` 为上升。

机械臂预设位置只有初始位置可以使用，和复位键是同一个位置，其余位置不要点击发送





##### 点击复位按钮则能复位机械臂和灵巧手到初始位置（复位前注意运动过程中不会碰撞）



##### 结束实验后，点击关闭系统关闭所有后台程序，确认全部程序关闭后

##### 关闭显控网页界面，并给各设备（机械臂电源、灵巧手电池）断电

## Debug

（预留，手动补充）

## 顶部按钮

- `切换到 Debug`：进入调试界面，显示隐藏的节点、网络、日志和主手/Simulink 相关调试项。
- `切换到 Main`：从 Debug 返回主界面。

## 系统控制

- `板卡重启`：通过 SSH 向 Jetson 发送重启命令。
- `初始化`：拉起系统所需节点和本地程序，包括 Jetson 节点、UDP 桥接、本地灵巧手程序和数据手套程序。
- `复位`：机械臂回到零位目标，灵巧手回到复位位置。
- `关闭系统`：停止 Jetson 节点、UDP 桥接和本地手/手套程序。
- `急停`：发送停止命令，机械臂和灵巧手保持当前状态，节点不关闭。

## 本地控制输入

- `键盘`：使用键盘作为机械臂输入源。
- `手柄`：使用 USB 有线手柄作为机械臂输入源。
- `位置控制`：按末端 XYZ 位置目标控制。
- `关节控制`：按三个关节目标控制。
- `W/A/S/D`：键盘模式下控制前后左右。
- `Q/E`：键盘模式下控制上下。
- `左摇杆`：手柄模式下控制前后左右。
- `L/L2`：手柄模式下控制上下，`L` 为下降，`L2` 为上升。

## 机械臂预设位置

- `初始位置 - 发送`：按当前输入框角度发送初始位置目标。
- `预抓取 - 发送`：按当前输入框角度发送预抓取目标。
- `抓取姿态 - 发送`：按当前输入框角度发送抓取目标。
- `释放姿态 - 发送`：按当前输入框角度发送释放目标。

## 灵巧手 / 数据手套

- `启动灵巧手 exe`：启动本地灵巧手控制程序。
- `重启灵巧手 exe`：关闭并重新启动本地灵巧手控制程序。
- `灵巧手状态`：检查灵巧手程序状态。
- `启动 Unity 数据手套`：启动本地数据手套程序。
- `数据手套状态`：检查数据手套程序状态。
- `链路状态`：检查本机手/手套 UDP 链路。
- `手套操控模式`：切换灵巧手到手套随动模式。
- `预编程位置模式`：切换灵巧手到预设位置控制模式。
- `复位 2000`：灵巧手全部关节回到 2000。
- `半握`：发送半握预设。
- `握拳`：发送握拳预设。
- `捏取`：发送捏取预设。
- `钩拉`：发送钩拉预设。
- `发送自定义位置`：按 6 个输入框中的数值发送灵巧手位置。
